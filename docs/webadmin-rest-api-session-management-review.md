...
WebAdmin, REST API 및 동일 PC의 여러 브라우저에서 로그인이 동시에 발생해도 공통 사용자 한도가 일관되게 적용되도록 `CreateUserSessionCommand`의 **세션 수 확인과 신규 세션 등록을 하나의 임계 구역**으로 처리한다. 기존에는 두 요청이 동시에 현재 세션 수를 읽으면 양쪽 모두 여유가 있다고 판단하여 설정된 한도를 초과할 가능성이 있었다.

보강된 처리는 공통 `SessionDataContainer`를 잠근 상태에서 다음 순서로 실행한다.

1. 동일 `DbUser.id`의 유효 세션 수를 조회한다.
2. 통합 한도에 도달했으면 접속 경로, 브라우저 또는 source IP와 관계없이 생성을 거부한다.
3. 여유가 있으면 Engine session ID를 만들고 사용자 세션을 등록한다.
4. 등록이 완료된 후에 다음 WebAdmin 또는 REST API 로그인 요청이 한도를 확인하도록 한다.

이 변경은 서로 다른 브라우저의 인증 쿠키를 하나로 공유하거나 물리적 PC를 신뢰 식별자로 사용하지 않는다. IP 기반 세션 병합은 NAT·프록시 환경에서 서로 다른 사용자의 세션을 오인하거나 세션 탈취로 이어질 수 있으므로 적용하지 않는다. 대신 모든 접속 경로가 생성하는 **서로 독립된 세션을 동일 사용자 한도에서 원자적으로 합산·제한**한다.
