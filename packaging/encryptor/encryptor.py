...
def _looks_like_plaintext_config(data):
    """Distinguish expected text config from opaque legacy ciphertext."""
    if b"\x00" in data:
        return False
    try:
    ...
    return bool(meaningful) and all("=" in line for line in meaningful)
...
def atomic_update_config(path, config, allowed_roots=ALLOWED_ROOTS):
    """Persist non-secret metadata with the same atomicity as ciphertext."""
    path = Path(path)
    if path.exists():
        info = validate_ovirt_path(path, allowed_roots=allowed_roots)
...
def transform_file(source, output, passphrase, decrypt=False, config=None,
    ...
    if decrypt:
        ...
        plaintext = data
        legacy = (config or {}).get("legacy_cbc")
        if isinstance(legacy, dict) and legacy.get("enabled", False):
            if not _looks_like_plaintext_config(data):
                plaintext = decrypt_legacy_cbc(data, config)
                if not _looks_like_plaintext_config(plaintext):
                    raise EncryptorError(
                        "Legacy CBC result is not a valid configuration file"
                    )
        result = encrypt_bytes(plaintext, passphrase)
        # Verify before replacing any persistent file.
        if decrypt_gcm_bytes(result, passphrase) != plaintext:
