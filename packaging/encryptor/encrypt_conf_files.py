...
def main(argv=None):
    ...
    parser.add_argument(
        "--acknowledge-runtime-decryption",
        action="store_true",
        help=(
            "confirm that Engine launcher and AAA runtime decryption "
            "have been deployed and tested"
        ),
    )
    args = parser.parse_args(argv)
    if not args.acknowledge_runtime_decryption:
        parser.error(
            "refusing to encrypt runtime configuration without "
            "--acknowledge-runtime-decryption"
        )
    try:
